...
def nro_upas(path: str) -> pd.DataFrame:
    ...
    pattern = re.compile("|".join(CONVERTER_MES_SHORT.keys()))
    f = lambda x: CONVERTER_MES_SHORT[re.escape(x.group(0))]

    df = df.assign(
        id_municipio_6digitos=df["Município"].str.slice(0, 6),
...
def nro_samu(path: str) -> pd.DataFrame:
    """Converte a base extraida do DataSUS para um formato utilizavel na regressão.
    Caso haja duplicidade de reportes para um mesmo mês, adotamos como correto o
    maior número informado. O ID do município não é o mesmo utilizado pelo IBGE.
...
def nro_mortes_local(path: str) -> pd.DataFrame:
    ...
    df = df.assign(
        ...
        mortes_outro_estab_saude=pd.to_numeric(
            df["Outro estabelecimento de saúde"], errors="coerce"
        ).fillna(0, downcast="int"),
        mortes_domicilio=pd.to_numeric(df["Domicílio"], errors="coerce").fillna(
            0, downcast="int"
        ),
        mortes_via_publica=pd.to_numeric(df["Via pública"], errors="coerce").fillna(
            0, downcast="int"
        ),
...
def consolidar_nro_mortes_local(folder: str) -> pd.DataFrame:
    ...
    for file in files:
        f = nro_mortes_local(folder + file)
...
def nro_mortes_mes(path: str) -> pd.DataFrame:
    ...
    converter = dict((re.escape(k), v) for k, v in CONVERTER_MES_LONG.items())
...
def populacao_sp(path: str) -> pd.DataFrame:
    ...
    A query utilizada no BigQuery para extrair os dados está no arquivo ETL.sql.
...
def base_saneamento(path: str) -> pd.DataFrame:
    ...
    A forma de extração utilizada dentro do SNIS é:
        ...
        Série Histórica > Municípios > Informações e indicadores municipais consolidados >
        Ano de Referência = 2010 - 2020 > Estado = São Paulo > Municípios = Todos >
    ...
    df = pd.read_excel(
        path, dtype={"Código do Município": str, "Ano de Referência": int}
    )
...
def criar_variaveis_regressao(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.assign(
        ...
        mortes_outro_estab_saude_pc=((df["mortes_outro_estab_saude"] / df["populacao"]) * 100000),
        ...
        mortes_outros_locais_pc=((df["mortes_outros_locais"] / df["populacao"]) * 100000),
        mortes_info_ignorada_pc=((df["mortes_info_ignorada"] / df["populacao"]) * 100000),
    )
...
if __name__ == "__main__":
    ...
    df_populacao = populacao_sp("dados/bases_origem/populacao_sp.csv")
    df_pib = pib_municipios("dados/bases_origem/pib_municipios.csv")
    df_mortes = consolidar_nro_mortes_ano("dados/bases_origem/mortes_por_mes_sp/")
    df_upas = nro_upas("dados/bases_origem/upas_sp.csv")
    df_samu = nro_samu("dados/bases_origem/samu_municipios.csv")
    df_gov = bases_politicas("dados/bases_origem/partidos_vencedores_governador.csv")
    df_pref = bases_politicas(
        "dados/bases_origem/partidos_vencedores_prefeitos.csv", type="pref"
    )
    df_saneamento = base_saneamento("dados/bases_origem/saneamento_municipio.xlsx")
    df_mortes_local = consolidar_nro_mortes_local(
        "dados/bases_origem/mortes_por_local_sp/"
    )
    ...
    df.to_excel("base_regressao.xlsx", index=False)
